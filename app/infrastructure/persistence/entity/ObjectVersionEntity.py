...
class ObjectVersionEntity(Base):
    ...
    version_id: Mapped[bytes] = mapped_column(
        LargeBinary(24),
        primary_key=True,
    )
    ...
    created_by_identity_id: Mapped[bytes] = mapped_column(
        LargeBinary(24),
        nullable=False,
    )

    created_by_subject_type: Mapped[str] = mapped_column(
        String(30),
        nullable=False,
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "object_id",
            "version_number",
            name="uq_object_version_number",
        ),
        Index(
            "ix_object_version_object",
            "object_id",
        ),
    )