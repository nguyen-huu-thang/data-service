...
class DataObjectEntity(TimestampMixin, Base):
    ...
    owner_identity_id: Mapped[bytes] = mapped_column(
        LargeBinary(24),
        nullable=False,
    )

    owner_subject_type: Mapped[str] = mapped_column(
        String(30),
        nullable=False,
    )
    ...
    current_version_id: Mapped[bytes | None] = mapped_column(
        LargeBinary(24),
        nullable=True,
    )
    ...
    __table_args__ = (
        Index(
            "ix_data_object_owner_identity",
            "owner_identity_id",
        ),
        Index(
            "ix_data_object_owner_status",
            "owner_identity_id",
            "status",
        ),
        Index(
            "ix_data_object_owner_type",
            "owner_identity_id",
            "object_type",
        ),
        Index(
            "ix_data_object_shard",
            "shard_id",
        ),
        Index(
            "ix_data_object_status",
            "status",
        ),
    )