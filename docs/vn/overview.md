...
Khi object được tạo, placement-service chọn cho nó một data shard dựa trên **dung lượng & tải** của các shard (KHÔNG băm từ `identity_id`). `shard_id` được chọn rồi **lưu cố định cùng bản ghi** object — việc gán này là vĩnh viễn, object không bao giờ chuyển sang shard khác.

```
object mới → placement (dung lượng & tải) → shard_id → lưu cùng object  (cố định mãi mãi)
